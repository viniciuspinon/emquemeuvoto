...
def test_governismo_e_pesquisas_sao_validos():
    """Garante que pesquisas eleitorais, quando presentes, tenham estrutura e valores percentuais válidos e governismo seja percentual oficial válido."""
    resp = client.get("/api/candidatos?limite=50")
    assert resp.status_code == 200
    candidatos = resp.json()["candidatos"]
    
    for c in candidatos:
        if c["pesquisas"] is not None:
            assert isinstance(c["pesquisas"], list)
            for p in c["pesquisas"]:
                assert 0.0 <= p["percentual"] <= 100.0
                assert p["instituto"]
        if c["governismo_pct"] is not None:
            assert 0.0 <= c["governismo_pct"] <= 100.0
