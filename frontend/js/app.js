...
const App = {
  ...
  async init() {
    ...
    try {
      const allCols = [
        ...
        "mandato", "pesquisas", "situacao", "profissao", "raca", "idade", "estado_civil",
      ...
      this.getPesquisasPresidenciaData().catch(() => {});
                ...
                <div class="card-top-row">
                  ...
                  <div class="card-meta-col">
                    ...
                    <div class="card-sub-tags-row">
                      ${(() => {
                        const pesqs = Array.isArray(c.pesquisas) ? c.pesquisas : (typeof c.pesquisas === 'string' ? (() => { try { return JSON.parse(c.pesquisas); } catch(e) { return []; } })() : []);
                        let pos = null;
                        let pct = null;
                        if (pesqs && pesqs.length > 0 && pesqs[0]) {
                          const p = pesqs[0];
                          pos = p.posicao != null ? p.posicao : (p.posicao_ranking != null ? p.posicao_ranking : (p.ranking != null ? p.ranking : null));
                          pct = p.percentual != null ? Number(p.percentual) : (p.media_ponderada_pct != null ? Number(p.media_ponderada_pct) : null);
                        }
                        const cargoUp = String(c.cargo || '').toUpperCase();
                        if (pos == null && cargoUp.includes('PRESIDENTE') && App._pesquisasPresidenciaCache && App._pesquisasPresidenciaCache.series) {
                          const cleanSq = String(c.sq_candidato || '').trim();
                          const cleanParty = String(c.partido || '').toUpperCase().trim();
                          const cleanUrna = String(c.nome_urna || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
                          const s = App._pesquisasPresidenciaCache.series.find(x => String(x.sq_candidato || '').trim() === cleanSq) ||
                            App._pesquisasPresidenciaCache.series.find(x => String(x.partido || '').toUpperCase().trim() === cleanParty) ||
                            App._pesquisasPresidenciaCache.series.find(x => {
                              const xName = String(x.candidato || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
                              return xName.includes(cleanUrna) || cleanUrna.includes(xName);
                            });
                          if (s) {
                            pos = s.posicao_ranking || s.posicao;
                            pct = s.media_atual;
                          }
                        }
                        if (pos == null && cargoUp.includes('GOVERNADOR') && c.uf && App._pesquisasGovCache && App._pesquisasGovCache[c.uf]) {
                          const cleanSq = String(c.sq_candidato || '').trim();
                          const cleanParty = String(c.partido || '').toUpperCase().trim();
                          const cleanUrna = String(c.nome_urna || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
                          const s = App._pesquisasGovCache[c.uf].find(x => String(x.sq_candidato || '').trim() === cleanSq) ||
                            App._pesquisasGovCache[c.uf].find(x => {
                              const xParty = String(x.partido || '').toUpperCase().trim();
                              const xName = String(x.candidato || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
                              return (xParty === cleanParty && (xName.includes(cleanUrna) || cleanUrna.includes(xName)));
                            });
                          if (s) {
                            pos = s.posicao_ranking || s.posicao;
                            pct = s.media_ponderada_pct;
                          }
                        }

                        if (pos != null || pct != null) {
                          const posTxt = pos != null ? `${pos}º nas pesquisas` : 'Nas pesquisas';
                          return `<button type="button" class="card-sub-badge card-poll-badge" style="background: rgba(59, 130, 246, 0.12); color: var(--accent-blue); font-weight: 700; border: 1px solid rgba(59, 130, 246, 0.25); cursor: pointer;" title="Clique para ver o desempenho completo nas pesquisas eleitorais" onclick="event.stopPropagation(); App.openDetails('${c.sq_candidato}', 'tabPesquisas');">📊 ${posTxt} ↗</button>`;
                        }
                        return '';
                      })()}
                      ${c.plano_governo_url ? `
  ...
  sortTableBy(colKey) {
    ...
    } else if (colKey === "pesquisas") {
      nextSort = (cur === "pesquisas") ? "pesquisas_asc" : "pesquisas";
    } else if (colKey === "bens") {
  ...
  renderTableView(container) {
    ...
    const getArrow = (colKey) => {
      if (colKey === "nome") {
        ...
        if (curOrd === "pesquisas") return '<span class="sort-arrow">▼</span>';
        if (curOrd === "pesquisas_asc") return '<span class="sort-arrow">▲</span>';
      } else if (colKey === "bens") {
    ...
    const isAct = (colKey) => {
      ...
      if (colKey === "pesquisas") return curOrd === "pesquisas" || curOrd === "pesquisas_asc";
        ...
        return `
          <th class="draggable-col" ${dragAttrs} style="${getWStyle(130)} text-align: right;">
            <div class="th-content-wrapper right">
              ${dragHandle}
              <span class="th-sort-title ${isAct('pesquisas') ? 'active' : ''}" onclick="App.sortTableBy('pesquisas')" title="Ordenar por % nas Pesquisas Eleitorais" draggable="false">Pesquisas ${getArrow('pesquisas')}</span>
              <button type="button" class="btn-table-col-filter ${TableFilterManager.hasFilter('pesquisas') ? 'active-filter' : ''}" onclick="event.stopPropagation(); TableFilterManager.toggle(this, 'pesquisas', event)" title="Filtrar por Pesquisas" draggable="false">⚙️</button>
            </div>
            ${resizer}
          </th>`;
      case "situacao":
  ...
  renderTableTd(colKey, c) {
    ...
    switch (colKey) {
      ...
      case "pesquisas": {
        const rawP = Array.isArray(c.pesquisas) ? c.pesquisas : (typeof c.pesquisas === 'string' ? (() => { try { return JSON.parse(c.pesquisas); } catch (e) { return []; } })() : []);
        const p = rawP.length > 0 ? rawP[0] : null;
        if (p && p.percentual !== null && p.percentual !== undefined) {
          const pctStr = Number(p.percentual).toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 });
          const posStr = p.posicao ? `(${p.posicao}º)` : '';
          return `
            <td style="text-align: right;">
              <span style="font-family: var(--font-mono); font-weight: 700; color: var(--accent-petroleo); font-size: 0.8rem;">
                ${pctStr}% ${posStr ? `<span style="font-weight: 600; font-size: 0.72rem; color: var(--text-muted); margin-left: 2px;">${posStr}</span>` : ''}
              </span>
            </td>`;
        }
        return `<td style="text-align: right;"><span style="color: var(--text-muted); font-size: 0.72rem;">---</span></td>`;
      }
      case "situacao":
        return `
          <td>
  ...
  openExportModal() {
    ...
    this.setExportScope(this.exportState.scope || "all");
    this.setExportFormat(this.exportState.format || "csv");
  ...
  setExportScope(scope) {
    ...
    if (activeEl) {
      ...
      const radio = activeEl.querySelector("input");
  ...
  async executarDownloadExportacao() {
    ...
    const confirmBtn = document.querySelector(".btn-export-confirm");
    const originalText = confirmBtn ? confirmBtn.innerHTML : "";

    if (scope === "all") {
      if (confirmBtn) {
        confirmBtn.disabled = true;
        confirmBtn.innerHTML = `<span>⏳ Baixando ${(this.state.total || 0).toLocaleString("pt-BR")} candidatos...</span>`;
      }
      ...
      const params = new URLSearchParams({
        ...
        ideologia_max: this.state.ideologia_max,
        pagina: 1,
        limite: Math.max(1, Math.min(30000, this.state.total || 25000)),
        ordenacao: this.state.ordenacao
      });
      ...
      if (this.state.selectedCargos && this.state.selectedCargos.length > 0) {
        params.append("cargo", this.state.selectedCargos.join(","));
      } else if (this.state.cargo) {
        params.append("cargo", this.state.cargo);
      }
      ...
      if (this.state.busca) params.append("busca", this.state.busca);
      if (this.state.partidos_gosta && this.state.partidos_gosta.length > 0) params.append("partidos_gosta", this.state.partidos_gosta.join(","));
      if (this.state.partidos_desgosta && this.state.partidos_desgosta.length > 0) params.append("partidos_desgosta", this.state.partidos_desgosta.join(","));
      ...
      try {
        ...
        if (resp.ok) {
          ...
          console.error("Falha ao exportar dados completos da API:", resp.status, await resp.text());
          alert("Erro ao buscar a base completa de candidatos no servidor (" + resp.status + ").");
          return;
        }
      } catch (e) {
        console.error("Erro ao buscar todos os dados para exportação:", e);
        alert("Erro de conexão ao buscar todos os dados para exportação.");
        return;
      } finally {
        if (confirmBtn) {
    ...
    if (!dados || dados.length === 0) {
      alert("Nenhum candidato encontrado para exportar com os filtros atuais.");
      return;
    }
  ...
  _pesquisasPresidenciaCache: null,
  _pesquisasGovCache: {},
  _pesquisasGovHistoricoCache: {},

  async getPesquisasPresidenciaData() {
    if (this._pesquisasPresidenciaCache) return this._pesquisasPresidenciaCache;
    try {
      const res = await fetch("/api/pesquisas/presidencia");
      if (res.ok) {
        this._pesquisasPresidenciaCache = await res.json();
        return this._pesquisasPresidenciaCache;
      }
    } catch (e) {
      console.warn("Erro ao buscar pesquisas presidência:", e);
    }
    return null;
  },

  async getPesquisasGovernadoresData(uf) {
    const cleanUf = (uf || "").toUpperCase().trim();
    if (cleanUf && this._pesquisasGovCache[cleanUf]) return this._pesquisasGovCache[cleanUf];
    try {
      const url = cleanUf ? `/api/pesquisas/governadores?uf=${cleanUf}` : "/api/pesquisas/governadores";
      const res = await fetch(url);
      if (res.ok) {
        const data = await res.json();
        if (cleanUf) this._pesquisasGovCache[cleanUf] = data;
        return data;
      }
    } catch (e) {
      console.warn("Erro ao buscar pesquisas governadores:", e);
    }
    return null;
  },

  async getPesquisasGovernadorHistoricoData(uf) {
    const cleanUf = (uf || "").toUpperCase().trim();
    if (!cleanUf) return null;
    if (this._pesquisasGovHistoricoCache[cleanUf]) return this._pesquisasGovHistoricoCache[cleanUf];
    try {
      const res = await fetch(`/api/pesquisas/governadores/historico?uf=${cleanUf}`);
      if (res.ok) {
        const data = await res.json();
        this._pesquisasGovHistoricoCache[cleanUf] = data;
        return data;
      }
    } catch (e) {
      console.warn(`Erro ao buscar histórico de pesquisas governadores (${cleanUf}):`, e);
    }
    return null;
  },

  async openDetails(sq_candidato, initialTab = 'tabPerfil') {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        // Pré-carregar pesquisas completas com série temporal
        const cargoClean = String(c.cargo || '').toUpperCase().trim();
        if (cargoClean.includes("PRESIDENTE")) {
          const presData = await this.getPesquisasPresidenciaData();
          if (presData && presData.series) {
            const cleanSq = String(c.sq_candidato || '').trim();
            const cleanPartido = String(c.partido || '').toUpperCase().trim();
            const cleanUrna = String(c.nome_urna || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();

            c._presidenciaPesquisa = presData.series.find(s => String(s.sq_candidato || '').trim() === cleanSq) ||
              presData.series.find(s => String(s.partido || '').toUpperCase().trim() === cleanPartido) ||
              presData.series.find(s => {
                const sName = String(s.candidato || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
                return sName.includes(cleanUrna) || cleanUrna.includes(sName);
              });
            c._presidenciaMeta = presData;
          }
        } else if (cargoClean.includes("GOVERNADOR") && c.uf) {
          const [govList, govHist] = await Promise.all([
            this.getPesquisasGovernadoresData(c.uf),
            this.getPesquisasGovernadorHistoricoData(c.uf)
          ]);
          if (govList && Array.isArray(govList)) {
            c._govPesquisas = govList;
          }
          if (govHist && govHist.series) {
            const cleanSq = String(c.sq_candidato || '').trim();
            const cleanPartido = String(c.partido || '').toUpperCase().trim();
            const cleanUrna = String(c.nome_urna || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();

            c._govPesquisa = govHist.series.find(s => String(s.sq_candidato || '').trim() === cleanSq) ||
              govHist.series.find(s => String(s.partido || '').toUpperCase().trim() === cleanPartido) ||
              govHist.series.find(s => {
                const sName = String(s.candidato || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
                return sName.includes(cleanUrna) || cleanUrna.includes(sName);
              });
            c._govMeta = govHist;
        ...
        if (initialTab && initialTab !== 'tabPerfil') {
          const tabBtn = document.querySelector(`.drawer-tab-btn[onclick*="${initialTab}"]`);
          this.switchDrawerTab(initialTab, tabBtn);
        }
  ...
  renderDrawerDetails(c) {
    ...
    // Pesquisas Eleitorais (Agregador EXAME / Inteligov)
    const isPres = cargoStr.includes("PRESIDENTE");
    const isGov = cargoStr.includes("GOVERNADOR");
    const candColor = c.partido_cor_hex || App.getPartidoColor(c.partido);

    // Fallback se _presidenciaPesquisa não estiver preenchido
    if (isPres && (!c._presidenciaPesquisa || !c._presidenciaPesquisa.pontos || c._presidenciaPesquisa.pontos.length === 0) && App._pesquisasPresidenciaCache && App._pesquisasPresidenciaCache.series) {
      const cleanSq = String(c.sq_candidato || '').trim();
      const cleanPartido = String(c.partido || '').toUpperCase().trim();
      const cleanUrna = String(c.nome_urna || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
      c._presidenciaPesquisa = App._pesquisasPresidenciaCache.series.find(s => String(s.sq_candidato || '').trim() === cleanSq) ||
        App._pesquisasPresidenciaCache.series.find(s => String(s.partido || '').toUpperCase().trim() === cleanPartido) ||
        App._pesquisasPresidenciaCache.series.find(s => {
          const sName = String(s.candidato || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
          return sName.includes(cleanUrna) || cleanUrna.includes(sName);
        });
      c._presidenciaMeta = App._pesquisasPresidenciaCache;
    }

    if (isGov && (!c._govPesquisas || c._govPesquisas.length === 0) && c.uf && App._pesquisasGovCache && App._pesquisasGovCache[c.uf]) {
      c._govPesquisas = App._pesquisasGovCache[c.uf];
    }
    if (isGov && (!c._govPesquisa || !c._govPesquisa.pontos || c._govPesquisa.pontos.length === 0) && c.uf && App._pesquisasGovHistoricoCache && App._pesquisasGovHistoricoCache[c.uf] && App._pesquisasGovHistoricoCache[c.uf].series) {
      const govHist = App._pesquisasGovHistoricoCache[c.uf];
      const cleanSq = String(c.sq_candidato || '').trim();
      const cleanPartido = String(c.partido || '').toUpperCase().trim();
      const cleanUrna = String(c.nome_urna || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
      c._govPesquisa = govHist.series.find(s => String(s.sq_candidato || '').trim() === cleanSq) ||
    ...
    const rawPesquisas = Array.isArray(c.pesquisas)
      ? c.pesquisas
      : (typeof c.pesquisas === 'string' ? (() => { try { return JSON.parse(c.pesquisas); } catch (e) { return []; } })() : []);

    const pItem = rawPesquisas[0] || (c._presidenciaPesquisa ? {
      percentual: c._presidenciaPesquisa.media_atual,
      posicao: c._presidenciaPesquisa.posicao_ranking,
      instituto: "EXAME / Inteligov (Média Ponderada)",
      data: "Setembro de 2026",
      fonte: "Agregador EXAME / Inteligov"
    } : (c._govPesquisa ? {
      percentual: c._govPesquisa.media_atual,
      posicao: c._govPesquisa.posicao_ranking,
      instituto: "EXAME / Inteligov (Média Ponderada)",
      data: "Setembro de 2026",
      fonte: "Agregador EXAME / Inteligov"
    } : (c._govPesquisas && c._govPesquisas.find(g => String(g.sq_candidato) === String(c.sq_candidato)) ? (() => {
      const g = c._govPesquisas.find(x => String(x.sq_candidato) === String(c.sq_candidato));
      return { percentual: g.media_ponderada_pct, posicao: g.posicao_ranking, instituto: "EXAME / Inteligov (Média Ponderada)", data: g.data_periodo || "Setembro/2026", fonte: g.fonte || "Agregador EXAME / Inteligov" };
    })() : null)));

    const pct = pItem ? (pItem.percentual != null ? Number(pItem.percentual) : (pItem.media_ponderada_pct != null ? Number(pItem.media_ponderada_pct) : null)) : (c._presidenciaPesquisa ? Number(c._presidenciaPesquisa.media_atual) : (c._govPesquisa ? Number(c._govPesquisa.media_atual) : null));
    const pos = pItem ? (pItem.posicao != null ? pItem.posicao : (pItem.posicao_ranking != null ? pItem.posicao_ranking : (pItem.ranking != null ? pItem.ranking : null))) : (c._presidenciaPesquisa ? c._presidenciaPesquisa.posicao_ranking : (c._govPesquisa ? c._govPesquisa.posicao_ranking : null));

    const hasPesquisas = rawPesquisas.length > 0 || Boolean(c._presidenciaPesquisa) || Boolean(c._govPesquisa) || (Array.isArray(c._govPesquisas) && c._govPesquisas.length > 0) || pos != null || pct != null;
    ...
    body.innerHTML = `
      ...
      <div class="drawer-tabs">
        ...
        ` : ''}
        ${hasPesquisas ? `
          <button type="button" class="drawer-tab-btn" onclick="App.switchDrawerTab('tabPesquisas', this)">📊 ${exibirRadarFederal ? '4.' : '3.'} Desempenho nas Pesquisas</button>
        ` : ''}
      </div>
      ...
      <div id="tabPerfil" class="drawer-tab-content" style="padding-top: 0.5rem;">
        <div style="display: flex; flex-direction: column; gap: 0.55rem; font-size: 0.8rem;">
          ...
          <!-- Banner de Acesso Rápido às Pesquisas -->
          ${hasPesquisas ? `
            <div style="background: rgba(8, 76, 97, 0.08); border: 1.5px solid var(--accent-petroleo); border-radius: var(--radius-sm); padding: 0.75rem; margin-bottom: 0.5rem; display: flex; justify-content: space-between; align-items: center; cursor: pointer;" onclick="const btn = document.querySelector('.drawer-tab-btn[onclick*=\\'tabPesquisas\\']'); App.switchDrawerTab('tabPesquisas', btn);" title="Clique para abrir a aba de Desempenho nas Pesquisas com o gráfico completo">
              <div style="display: flex; align-items: center; gap: 0.6rem;">
                <span style="font-size: 1.35rem;">📊</span>
                <div>
                  <div style="font-size: 0.82rem; font-weight: 800; color: var(--accent-petroleo);">
                    ${pos ? `${pos}º Lugar nas Pesquisas Eleitorais` : 'Desempenho no Agregador de Pesquisas'} ${pct !== null && pct !== undefined ? `(${pct.toFixed(1)}%)` : ''}
                  </div>
                  <div style="font-size: 0.7rem; color: var(--text-secondary); margin-top: 1px;">
                    Média ponderada oficial • Clique aqui para ver o gráfico longitudinal e histórico
                  </div>
                </div>
              </div>
              <span style="font-size: 0.74rem; font-weight: 700; color: var(--accent-petroleo); background: var(--bg-page); padding: 0.25rem 0.6rem; border-radius: var(--radius-xs); border: 1px solid rgba(8, 76, 97, 0.2);">Ver gráfico ↗</span>
            </div>
          ` : ''}
      ...
      <!-- Aba 3: Atuação & Mandato (apenas se houver mandato federal com dados do Radar) -->
      ${exibirRadarFederal ? `
      ...
      <!-- Aba: Desempenho nas Pesquisas (Agregador EXAME / Inteligov) -->
      ${hasPesquisas ? (() => {
        const isPres = cargoStr.includes("PRESIDENTE");
        const isGov = cargoStr.includes("GOVERNADOR");
        const candColor = c.partido_cor_hex || App.getPartidoColor(c.partido);

        // Fallback robusto se _presidenciaPesquisa não estiver preenchido
        if (isPres && (!c._presidenciaPesquisa || !c._presidenciaPesquisa.pontos || c._presidenciaPesquisa.pontos.length === 0) && App._pesquisasPresidenciaCache && App._pesquisasPresidenciaCache.series) {
          const cleanSq = String(c.sq_candidato || '').trim();
          const cleanPartido = String(c.partido || '').toUpperCase().trim();
          const cleanUrna = String(c.nome_urna || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
        ...
        const formatPtData = (dataStr) => {
          ...
          if (parts.length === 3) {
            ...
            const qz = dia <= 15 ? '1ª' : '2ª';
            return `${qz} ${mNames[mes] || mes}`;
          }
          return dataStr;
        };

        const formatPtDataFull = (dataStr) => {
          if (!dataStr) return '';
          const parts = String(dataStr).split('-');
          if (parts.length === 3) {
            const dia = parseInt(parts[2], 10);
            const mes = parseInt(parts[1], 10);
            const mNames = ['', 'Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];
            const qz = dia <= 15 ? '1ª quinzena' : '2ª quinzena';
            return `${qz} ${mNames[mes]} (${parts[2]}/${parts[1]})`;
          }
          return dataStr;
        };

        const activeMeta = isPres ? c._presidenciaMeta : (isGov ? c._govMeta : null);
        const activeSeriesCand = isPres ? c._presidenciaPesquisa : (isGov ? c._govPesquisa : null);

        let chartSvgHtml = "";
        if (activeMeta && activeMeta.series && activeMeta.series.length > 0) {
          const allSeries = activeMeta.series.filter(s => s.pontos && s.pontos.length > 0);
          const currentSeries = activeSeriesCand || allSeries.find(s => String(s.sq_candidato || '').trim() === String(c.sq_candidato || '').trim()) || allSeries[0];
          
          // Coleta todos os valores para escala Y consistente
          let maxValRaw = 0;
          allSeries.forEach(s => {
            s.pontos.forEach(pt => {
              const val = Number(pt.media_ponderada_pct) || 0;
              if (val > maxValRaw) maxValRaw = val;
            });
          });
          const minVal = 0;
          let maxVal = Math.ceil((maxValRaw + 4) / 5) * 5;
          ...
          // Grid lines a cada 10%
          const yStepValues = [];
          for (let st = 0; st <= maxVal; st += 10) {
            yStepValues.push(st);
          }
          if (!yStepValues.includes(maxVal)) yStepValues.push(maxVal);

          const gridLines = yStepValues.map(val => {
            const y = padT + (1 - (val - minVal) / (maxVal - minVal)) * plotH;
            return `
              <line x1="${padL}" y1="${y.toFixed(1)}" x2="${svgW - padR}" y2="${y.toFixed(1)}" stroke="var(--border-color, #E2E8F0)" stroke-width="1" stroke-dasharray="3,3" />
              <text x="${padL - 6}" y="${(y + 3.5).toFixed(1)}" fill="var(--text-muted, #94A3B8)" font-size="9" text-anchor="end" font-family="var(--font-mono)">${val}%</text>
            `;
          }).join("");

          // Labels de X (Quinzenas)
          const basePts = (currentSeries && currentSeries.pontos && currentSeries.pontos.length > 0) ? currentSeries.pontos : (allSeries[0]?.pontos || []);
          const xStepIndices = [0, 4, 8, 11, basePts.length - 1].filter((idx, p, arr) => idx < basePts.length && arr.indexOf(idx) === p);
          const xLabels = xStepIndices.map(idx => {
            const pt = basePts[idx];
            if (!pt) return '';
            const x = padL + (idx / Math.max(1, basePts.length - 1)) * plotW;
            const shortLabel = pt.quinzena ? pt.quinzena.replace("ª quinzena", "ª").replace("/2026", "") : formatPtData(pt.data);
            return `
              <text x="${x.toFixed(1)}" y="${svgH - 10}" fill="var(--text-secondary, #64748B)" font-size="9.5" text-anchor="middle" font-family="var(--font-sans)">${shortLabel}</text>
            `;
          }).join("");

          // Linhas dos outros candidatos (desenhadas primeiro para ficarem ao fundo)
          const otherLinesHtml = allSeries.filter(s => s !== currentSeries && (s.candidato !== currentSeries?.candidato)).map(s => {
            const pColor = App.getPartidoColor(s.partido);
            const coords = s.pontos.map((pt, idx) => {
              const x = padL + (idx / Math.max(1, s.pontos.length - 1)) * plotW;
              const y = padT + (1 - ((Number(pt.media_ponderada_pct) || 0) - minVal) / (maxVal - minVal)) * plotH;
              return { x, y, pt, idx };
            });
            if (coords.length === 0) return '';
            const linePath = coords.map((pt, idx) => `${idx === 0 ? 'M' : 'L'} ${pt.x.toFixed(1)} ${pt.y.toFixed(1)}`).join(" ");
            const lastPt = coords[coords.length - 1];
            return `
              <g class="chart-other-series" style="cursor: pointer;" opacity="0.65">
                <path d="${linePath}" fill="none" stroke="${pColor}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                  <title>${s.candidato} (${s.partido}): ${Number(s.media_atual || 0).toFixed(1)}%</title>
                </path>
                <circle cx="${lastPt.x.toFixed(1)}" cy="${lastPt.y.toFixed(1)}" r="2.8" fill="${pColor}">
                  <title>${s.candidato} (${s.partido}): ${Number(s.media_atual || 0).toFixed(1)}%</title>
                </circle>
              </g>
            `;
          ...
          // Linha e Área do candidato em visualização (frente)
          let currentLineHtml = "";
          let coords = [];
          if (currentSeries && currentSeries.pontos && currentSeries.pontos.length > 0) {
            coords = currentSeries.pontos.map((pt, idx) => {
              const x = padL + (idx / Math.max(1, currentSeries.pontos.length - 1)) * plotW;
              const y = padT + (1 - ((Number(pt.media_ponderada_pct) || 0) - minVal) / (maxVal - minVal)) * plotH;
              return { x, y, pt, idx };
            });
            const linePath = coords.map((pt, idx) => `${idx === 0 ? 'M' : 'L'} ${pt.x.toFixed(1)} ${pt.y.toFixed(1)}`).join(" ");
            ...
            const dotsHtml = coords.map(crd => {
              const pVal = Number(crd.pt.media_ponderada_pct).toFixed(1);
              const isLast = crd.idx === coords.length - 1;
              const fullDate = crd.pt.quinzena || formatPtDataFull(crd.pt.data);
              return `
                <g class="chart-point-group" tabindex="0">
                  <circle cx="${crd.x.toFixed(1)}" cy="${crd.y.toFixed(1)}" r="${isLast ? 5 : 3.5}" fill="${isLast ? '#FFFFFF' : candColor}" stroke="${candColor}" stroke-width="${isLast ? 3 : 1.8}">
                    <title>${fullDate} - ${currentSeries.candidato}: ${pVal}%</title>
                  </circle>
                  ${isLast ? `
                    <rect x="${(crd.x - 24).toFixed(1)}" y="${(crd.y - 22).toFixed(1)}" width="48" height="16" rx="4" fill="${candColor}" />
                    <text x="${crd.x.toFixed(1)}" y="${(crd.y - 10).toFixed(1)}" fill="#FFFFFF" font-size="10" font-weight="800" text-anchor="middle" font-family="var(--font-mono)">${pVal}%</text>
                  ` : ''}
                </g>
              `;
            }).join("");
            ...
            currentLineHtml = `
              <defs>
                <linearGradient id="pesqGrad_${c.sq_candidato}" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stop-color="${candColor}" stop-opacity="0.32" />
                  <stop offset="100%" stop-color="${candColor}" stop-opacity="0.02" />
                </linearGradient>
              </defs>
              <path d="${areaPath}" fill="url(#pesqGrad_${c.sq_candidato})" />
              <path d="${linePath}" fill="none" stroke="${candColor}" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round" />
              ${dotsHtml}
            `;
          }

          // Legenda dos Candidatos
          const topCandidates = [...allSeries].sort((a, b) => (b.media_atual || 0) - (a.media_atual || 0));
          const legendHtml = `
            <div style="display: flex; flex-wrap: wrap; gap: 0.4rem; margin-top: 0.65rem; padding-top: 0.5rem; border-top: 1px solid var(--border-color-subtle);">
              ${topCandidates.slice(0, 8).map(s => {
                const isCur = s === currentSeries || s.candidato === currentSeries?.candidato;
                const pColor = App.getPartidoColor(s.partido);
                return `
                  <div style="display: inline-flex; align-items: center; gap: 0.35rem; font-size: 0.72rem; padding: 0.2rem 0.45rem; border-radius: 4px; ${isCur ? 'background: rgba(8, 76, 97, 0.1); border: 1.5px solid ' + pColor + '; font-weight: 700;' : 'background: var(--bg-page); border: 1px solid var(--border-color-subtle); color: var(--text-secondary);'}">
                    <span style="width: 8px; height: 8px; border-radius: 50%; background: ${pColor}; display: inline-block;"></span>
                    <span style="color: var(--text-primary);">${s.candidato}</span>
                    <span class="party-tag-pill" style="border-left-color: ${pColor}; font-size: 0.6rem; padding: 0.05rem 0.25rem;">${s.partido}</span>
                    <strong style="font-family: var(--font-mono); font-size: 0.72rem; color: ${isCur ? 'var(--accent-petroleo)' : 'var(--text-muted)'};">${Number(s.media_atual || 0).toFixed(1)}%</strong>
                    ${isCur ? '<span style="font-size: 0.65rem; color: var(--accent-petroleo);">★</span>' : ''}
                  </div>
                `;
              }).join('')}
            </div>
          ...
          const chartTitle = isPres 
            ? "📈 Gráfico Comparativo de Evolução das Pesquisas • Presidência da República (Março a Setembro/2026)" 
            : `📈 Gráfico Comparativo de Evolução das Pesquisas • Governo de ${activeMeta.estado || UF_NOMES[c.uf] || c.uf} (Março a Setembro/2026)`;

          chartSvgHtml = `
            <div class="finance-breakdown-card" style="margin-top: 0.5rem;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; border-bottom: 1px solid var(--border-color-subtle); padding-bottom: 0.35rem;">
                <div style="font-size: 0.74rem; font-weight: 700; color: var(--text-primary); text-transform: uppercase; letter-spacing: 0.04em;">
                  ${chartTitle}
                </div>
                <span style="font-size: 0.68rem; color: var(--text-muted); font-family: var(--font-mono);">
                  ${basePts.length} quinzenas • Multi-candidatos
                </span>
              </div>
              
              <div style="width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch;">
                <svg viewBox="0 0 ${svgW} ${svgH}" style="width: 100%; height: auto; max-height: 230px; display: block; overflow: visible;">
                  ${gridLines}
                  ${otherLinesHtml}
                  ${currentLineHtml}
                  ${xLabels}
        ...
        return `
          <div id="tabPesquisas" class="drawer-tab-content" style="display: none; padding-top: 0.5rem;">
            <div style="display: flex; flex-direction: column; gap: 0.75rem; font-size: 0.8rem;">
              
              <!-- Bloco de Destaque / KPIs Principais das Pesquisas -->
              <div class="finance-breakdown-card" style="border-top: 3px solid var(--accent-petroleo, #084C61);">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.4rem;">
                  <span style="font-size: 0.74rem; font-weight: 800; color: var(--accent-petroleo, #084C61); text-transform: uppercase; letter-spacing: 0.05em;">
                    Desempenho no Agregador de Pesquisas Eleitorais 2026
                  </span>
                  <span class="party-tag-pill" style="border-left-color: var(--accent-petroleo, #084C61); font-size: 0.65rem;">
                    ${c.cargo} • ${c.uf}
                  </span>
                ...
                <p style="font-size: 0.74rem; color: var(--text-secondary); margin-top: 0.35rem; line-height: 1.4;">
                  Média ponderada consolidada pelo <strong>Agregador EXAME / Inteligov</strong> para as Eleições 2026.
                </p>
              </div>
              ...
              <div class="finance-overview-grid">
                <div class="finance-kpi-card highlight">
                  <div class="finance-kpi-label">Intenção de Voto (Média)</div>
                  <div class="finance-kpi-value" style="color: var(--accent-petroleo, #084C61);">
                    ${pct !== null && pct !== undefined ? `${pct.toFixed(1)}%` : '---'}
                  </div>
                  <div style="width: 100%; height: 6px; background: rgba(0,0,0,0.08); border-radius: 3px; margin: 0.4rem 0 0.2rem; overflow: hidden;">
                    <div style="width: ${Math.min(100, (pct || 0) * 2)}%; height: 100%; background: var(--accent-petroleo, #084C61); border-radius: 3px;"></div>
                  </div>
                  <div style="font-size: 0.68rem; color: var(--text-muted);">
                    Média móvel ponderada por confiabilidade
                  </div>
                </div>

                <div class="finance-kpi-card">
                  <div class="finance-kpi-label">Posição no Cenário</div>
                  <div class="finance-kpi-value" style="color: ${pos === 1 ? '#10B981' : (pos === 2 ? '#3B82F6' : 'var(--text-primary)')};">
                    ${pos ? `${pos}º Lugar` : '---'}
                  </div>
                  <div style="font-size: 0.68rem; color: var(--text-muted); margin-top: 0.4rem;">
                    ${c.cargo === 'PRESIDENTE' ? 'Cenário Estimulado Nacional' : `Cenário Estadual (${c.uf})`}
                  </div>
                </div>
              </div>

              <!-- Gráfico Longitudinal SVG -->
              ${chartSvgHtml}

              <!-- Metodologia e Fonte Oficial -->
              <div class="finance-breakdown-card" style="background: var(--bg-page); border: 1px solid var(--border-color-subtle); padding: 0.75rem;">
                <div style="display: flex; align-items: flex-start; gap: 0.5rem;">
                  ...
                  <div>
                    ...
                    <p style="font-size: 0.7rem; color: var(--text-secondary); margin-top: 0.2rem; line-height: 1.45;">
                      <strong>Fonte:</strong> Agregador de Pesquisas EXAME / Inteligov (Média Móvel Ponderada). Os dados agregam as principais pesquisas registradas no TSE, ponderando histórico de acertos do instituto, tamanho amostral e proximidade da eleição.
                    </p>
                  </div>
                </div>
              </div>
  ...
  switchDrawerTab(tabId, btn) {
    ...
    if (btn) {
      ...
      const autoBtn = document.querySelector(`.drawer-tab-btn[onclick*="${tabId}"]`);
