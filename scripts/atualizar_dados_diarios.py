...
def gravar_metadados_atualizacao(sucesso: bool, detalhes: dict):
    """Grava arquivo JSON com metadados da última atualização e sincroniza o index.html."""
    ...
    # Sincronizar automaticamente os metadados no frontend/index.html
    caminho_index = PROJECT_ROOT / "frontend" / "index.html"
    if caminho_index.exists():
        try:
            import re
            conteudo = caminho_index.read_text(encoding="utf-8")
            conteudo = re.sub(r'<strong id="headerUpdatedDate">[^<]+</strong>', f'<strong id="headerUpdatedDate">{data_str}</strong>', conteudo)
            conteudo = re.sub(r'id="headerUpdatedTime">às [^<]+<', f'id="headerUpdatedTime">às {hora_str}<', conteudo)
            conteudo = re.sub(r'<span id="headerLastUpdated"[^>]*>[^<]+</span>', f'<span id="headerLastUpdated" style="display: none;">{data_str} às {hora_str}</span>', conteudo)
            conteudo = re.sub(r'<span id="footerLastUpdated">Atualizado em [^<]+</span>', f'<span id="footerLastUpdated">Atualizado em {data_str} às {hora_str}</span>', conteudo)
            if detalhes.get("total_candidatos"):
                total_fmt = f"{detalhes['total_candidatos']:,}".replace(",", ".")
                conteudo = re.sub(r'<span class="stats-big-number" id="statsBigNumber">[^<]+</span>', f'<span class="stats-big-number" id="statsBigNumber">{total_fmt}</span>', conteudo)
            caminho_index.write_text(conteudo, encoding="utf-8")
            print(f"{GREEN}✓ Data e métricas sincronizadas em:{RESET} {caminho_index.name}")
        except Exception as e:
            print(f"{YELLOW}Aviso: Não foi possível sincronizar index.html: {e}{RESET}")
