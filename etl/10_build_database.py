...
def build_database():
    ...
    # Garantir que colunas críticas analíticas existam mesmo se o parquet não tiver sido previamente enriquecido
    ...
    con.execute("ALTER TABLE candidatos ADD COLUMN IF NOT EXISTS historico_eleicoes VARCHAR DEFAULT NULL;")
    con.execute("ALTER TABLE candidatos ADD COLUMN IF NOT EXISTS contexto_proporcional VARCHAR DEFAULT NULL;")
    con.execute("ALTER TABLE candidatos ADD COLUMN IF NOT EXISTS emendas_parlamentares VARCHAR DEFAULT NULL;")
    con.execute("ALTER TABLE candidatos ADD COLUMN IF NOT EXISTS pesquisas VARCHAR DEFAULT NULL;")
    con.execute("ALTER TABLE candidatos ADD COLUMN IF NOT EXISTS chapa_info VARCHAR DEFAULT NULL;")
    ...
    # 8. Agregador de Pesquisas Eleitorais (EXAME / Inteligov)
    try:
        import importlib
        etl16 = importlib.import_module("etl.16_agregador_pesquisas")
        etl16.executar_ingestao_pesquisas()
    except Exception as e:
        print(f"[Aviso] Não foi possível executar agregador de pesquisas: {e}")
